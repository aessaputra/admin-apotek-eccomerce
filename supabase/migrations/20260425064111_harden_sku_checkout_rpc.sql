...
-- with the service-role client so it can create the full order/payment/shipment
-- aggregate transactionally. Keep direct client access revoked and grant only
...
comment on function public.create_checkout_order_aggregate(
  ...
  'Transactional checkout aggregate. SECURITY DEFINER is required to snapshot products.sku after customer SKU column SELECT is revoked; execute is restricted to service_role and direct callers remain constrained by auth.uid() or private.is_admin().';
