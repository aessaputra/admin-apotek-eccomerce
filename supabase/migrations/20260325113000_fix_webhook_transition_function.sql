begin;
drop function if exists public.apply_midtrans_webhook_transition(
  text,
  text,
  uuid,
  text,
...
create function public.apply_midtrans_webhook_transition(
  p_provider text,
  p_event_key text,
  p_order_id uuid,
...
begin
  ...
  where o.id = p_order_id
    and (
      (p_next_payment_status = 'pending' and o.payment_status is null)
...
comment on function public.apply_midtrans_webhook_transition is
  'Atomically inserts webhook idempotency marker and applies guarded Midtrans order status transition. Handles: pending (initial), settlement (success), authorize (reserved), deny/cancel/expire (failure), refund/partial_refund/chargeback/partial_chargeback (refunds).';
commit;
