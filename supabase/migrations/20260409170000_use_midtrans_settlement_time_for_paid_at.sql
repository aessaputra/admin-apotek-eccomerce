...
begin
  ...
  set
    ...
    paid_at = case
      when p_next_payment_status = 'settlement' then coalesce(o.paid_at, p_paid_at, timezone('utc'::text, now()))
      else o.paid_at
    end,
  ...
  where o.id = p_order_id
    and (
      ...
      or (p_next_payment_status = 'settlement' and o.payment_status in ('pending', 'authorize', 'deny'))
...
comment on function public.apply_midtrans_webhook_transition is
  'Atomically inserts webhook idempotency marker and applies guarded Midtrans order status transition. Settlement prefers the provider settlement timestamp when persisting orders.paid_at.';
