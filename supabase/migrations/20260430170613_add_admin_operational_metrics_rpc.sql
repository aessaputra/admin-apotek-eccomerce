...
comment on function public.admin_operational_metrics(text, date, date) is
  'Admin-only operational dashboard metrics bucketed by day, week, month, or year using Asia/Jakarta order-created dates; missing buckets are zero-filled and revenue counts settlement payments only.';
