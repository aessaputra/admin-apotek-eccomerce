  ...
  try {
    ...
    if (body.action === 'transition_status') {
      ...
      if (to === 'shipped' && !nextWaybill) {
        return new Response(JSON.stringify({ error: 'waybill_number is required for shipped status' }), {
          status: 400,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        })
      }
      ...
      if (
        ...
        !body.payload?.waybill_override_reason?.trim()
      ) {
        return new Response(
          JSON.stringify({ error: 'waybill_override_reason is required for manual waybill override' }),
          {
            status: 400,
            headers: { ...corsHeaders, 'Content-Type': 'application/json' },
          },
      ...
      await adminClient.from('order_activities').insert({
        ...
        actor_id: userId,
        actor_type: 'admin',
        metadata: { notes: body.payload?.notes ?? null, waybill: nextWaybill, waybill_source: body.payload?.waybill_source ?? null, override_reason: body.payload?.waybill_override_reason ?? null },
      })
    ...
    if (body.action === 'sync_tracking') {
      ...
      const trackingResp = await fetch(`${BITESHIP_BASE_URL}/trackings/${order.biteship_order_id}`, {
        method: 'GET',
        headers: {
      ...
      if (!trackingResp.ok) {
        ...
        return new Response(JSON.stringify({ error: 'Biteship tracking failed', details: errorBody }), {
