...
function createRequest(body: Record<string, unknown>, authorization = "Bearer token") {
...
function createCheckoutClientMock(options?: {
  ...
  const client: CheckoutAdminClient = {
    rpc,
  };
...
describe("createCheckoutOrderHandler", () => {
  ...
  it("returns 500 when checkout RPC returns an error", async () => {
    const { client, rpc } = createCheckoutClientMock({
